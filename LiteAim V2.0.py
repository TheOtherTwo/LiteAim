...
class Game:
        ...
        if self.m8_flash_active:
            ...
            if now >= self.m8_next_flash and in_center:
                ...
                if self.m8_flash_side == "left":
                    fx = random.randint(safe_w // 2, max(safe_w // 2 + 1, safe_w - 14))
                else:
                    fx = random.randint(min(WIDTH - safe_w + 14, WIDTH - safe_w // 2), WIDTH - safe_w // 2)
    ...
    def draw_menu(self):
        ...
        if len(self.buttons) >= 8:
            panel = self.buttons[0].rect.union(self.buttons[7].rect).inflate(36, 32)
